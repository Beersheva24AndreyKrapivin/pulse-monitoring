...
public class AppReducer {
    ...
    private static final String DEFAULT_SAVER_CLASS_NAME = "telran.monitoring.LatestValuesSaverMap";
    ...
    public AppReducer() {
        ...
        try {
            ...
            dataStream = (MiddlewareDataStream<SensorData>) MiddlewareDataStreamFactory.getStream(streamClassName,
                    streamName);
            String saverClassName = env.getOrDefault("SAVER_CLASS_NAME", DEFAULT_SAVER_CLASS_NAME);
            logger.log("finest", saverClassName);
            latestValuesSaver = LatestValuesSaver.getLatestValuesSaver(saverClassName, logger);
        } catch (Exception e) {
    ...
    private SensorData avgReducing(SensorData sensorData) {
        ...
        if (!latestData.isEmpty()) {
            long lastTimestamp = latestData.get(latestData.size() - 1).timestamp(); //getLast().timestamp();
