...
public class DataSourceSqlEmail extends DataSourceSQL {
    ...
    protected DataSourceSqlEmail() {
        super(String.format("select %s from %s where %s = (select %s from %s where %s = ?)",
                EMAIL_ADDRESS, NOTIFICATION_GROUPS_TABLE,
                NOTIFICATION_GROUP_ID, PATIENTS_TABLE, PATIENT_ID));
    }
